...
The streamlined code structure, avoiding traditional `.cuh` and `.cu` files and `Make`, is more practical for these environments,
...
Optimal Settings: The program was tested with L = 256, IT = 2e9, and NTHREADS = 128. These settings are known to provide a high confidence of convergence in simulations.
